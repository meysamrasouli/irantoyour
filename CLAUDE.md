...
| `hooks/useSlide.ts` | انیمیشن باز/بسته imperative با اندازه‌گیری دستی ارتفاع (جایگزین `useSlideToggle` قدیمی Vue) — **فعلاً هیچ‌جا استفاده نمی‌شه** (نه باگ، عمداً نگه‌داشته شده برای مواردی که کنترل دقیق‌تر duration/easing/callback لازم بشه). **استاندارد فعلی پروژه برای toggle/accordion، کلاس CSS `slide-toggle-container`/`slide-toggle-wrapper` (تریک `grid-template-rows: 0fr→1fr`, در `shared.scss`) هست، نه این hook** — نمونه‌ها: `arc_faq.tsx`, `profile/_navbar.tsx`. اگه توی یه لیست پاکسازی/حذف کد بلااستفاده بهش برخوردی، **قبل از حذف حتماً به کاربر یادآوری کن** که عمداً نگه داشته شده. |
...
- سرویس پیامکی هم هنوز انتخاب نشده (`OtpSMSJob::dispatch` در `OtpController` عمداً
  کامنته) — **تا وقتی کاربر صراحتاً بانک/سرویس پیامک رو انتخاب نکرده، سراغ پیاده‌سازی یا
  اصلاح درگاه بانکی و ارسال واقعی پیامک نرو**، حتی اگه در ریویو به‌عنوان باگ/ناقص دیده بشه
...
- **تصحیح مهم:** برخلاف چیزی که قبلاً اینجا نوشته شده بود، جدول‌های `offer`/`demand`
  در دیتابیس MySQL (`irantoyour`) **از قبل با دستِ کاربر ساخته شدن** (نه از طریق
  migration). این ۹ جدول توی DB هستن ولی هیچ‌کدوم migration file یا Eloquent Model
  ندارن، پس روی یک نصب fresh (`php artisan migrate`) اصلاً ساخته نمی‌شن:
  `advertise_offers`, `advertise_demands`, `advertise_units` (پیوت پلی‌مورفیک
  `advertiseable_type/id` ↔ `unit_id`), `units` (واحد تولیدی/تجاری کاربر), `companies`,
  `memberships`, `wallets` (پلی‌مورفیک `walletable_type/id`), `poultries` (دسته‌بندی
  درختی با `parent_id`), `provinces` (استان/شهر با `parent_id`). ستون‌های هر جدول رو
  Claude با `DESCRIBE` مستقیم از DB واقعی استخراج کرده (نه حدسی) — قبل از ساختن
  migration/Model براشون، دوباره از DB واقعی چک شه که تغییر نکرده باشن.
  این جدول‌ها هنوز **در حال تغییرن** (طبق نیاز پروژه عوض می‌شن) — کاربر گفته فعلاً فقط
  **گزارش** بشه و کاری (ساخت migration/Model/تغییر ستون) روشون انجام نشه تا خودش بگه.
